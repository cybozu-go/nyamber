...
- Kubernetes versions:
  - `kind-image-ref` in [.github/workflows/ci.yaml](/.github/workflows/ci.yaml): You can check the available versions at <https://hub.docker.com/r/kindest/node/tags>.
...
- After saving the changes above, update `ENVTEST_K8S_VERSION` in [Makefile](/Makefile) to the latest patch version among the latest supported kubernetes minor versions listed by running `aqua i && aqua exec -- setup-envtest list` at the root of this repository. If the latest minor supported version is `1.30.Z`, find `1.30.Z+` from the output but not `1.31.Z`.
...
Install CLI tools with [aqua][] if you haven't already (see [docs/development.md](/docs/development.md) for aqua installation):

```sh
aqua i
```

Then, update nyamber by running the following `make` commands:
